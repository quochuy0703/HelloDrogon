...
namespace app_repositories::usermicroservice_repository
{
    ...
    drogon::Task<Model> getById(std::string id)
    {
        ...
        try
        {
            auto db = drogon::app().getDbClient();
            drogon::orm::CoroMapper<Model> mp(db);
            course = co_await mp.findOne(drogon::orm::Criteria(Model::Cols::_userid, id));
        }
        catch (const drogon::orm::DrogonDbException &ex)
        {
            LOG_ERROR << ex.base().what();
    ...
    {
        std::string idPrimaryKey = *course.getUserid();
        Model courseExist;
        bool result;
        try
        {
    ...
    drogon::Task<bool> remove(std::string id)
    {
        ...
        try
        {
            auto db = drogon::app().getDbClient();
            drogon::orm::CoroMapper<Model> mp(db);
            result = co_await mp.deleteBy(drogon::orm::Criteria(Model::Cols::_username, id));
            if (!result)
            {
