...
const std::string Course::Cols::_title = "title";
const std::string Course::Cols::_instructor_id = "instructor_id";
const std::string Course::primaryKeyName = "";
const bool Course::hasPrimaryKey = false;
...
{"title","std::string","character varying",50,0,0,0},
{"instructor_id","int32_t","integer",4,0,0,0}
};
...
Course::Course(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[2].empty() && pJson.isMember(pMasqueradingVector[2]))
    {
        ...
        {
            instructorId_=std::make_shared<int32_t>((int32_t)pJson[pMasqueradingVector[2]].asInt64());
        }
    }
}
...
void Course::setIdToNull() noexcept
{
    id_.reset();
    dirtyFlag_[0] = true;
}

const std::string &Course::getValueOfTitle() const noexcept
{
...
const std::shared_ptr<std::string> &Course::getTitle() const noexcept
{
    return title_;
}
void Course::setTitle(const std::string &pTitle) noexcept
{
    title_ = std::make_shared<std::string>(pTitle);
    dirtyFlag_[1] = true;
}
void Course::setTitle(std::string &&pTitle) noexcept
{
    title_ = std::make_shared<std::string>(std::move(pTitle));
    dirtyFlag_[1] = true;
}
void Course::setTitleToNull() noexcept
{
    title_.reset();
    dirtyFlag_[1] = true;
}
    ...
    const std::vector<std::string> &pMasqueradingVector) const
{
    ...
    if(pMasqueradingVector.size() == 3)
    {
        ...
        if(!pMasqueradingVector[2].empty())
        {
            if(getInstructorId())
            {
                ret[pMasqueradingVector[2]]=getValueOfInstructorId();
            }
            else
            {
...
bool Course::validateJsonForCreation(const Json::Value &pJson, std::string &err)
{
    ...
    if(pJson.isMember("instructor_id"))
    {
        if(!validJsonOfField(2, "instructor_id", pJson["instructor_id"], err, true))
            return false;
    }
    return true;
}
...
bool Course::validateJsonForUpdate(const Json::Value &pJson, std::string &err)
{
    ...
    if(pJson.isMember("instructor_id"))
    {
        if(!validJsonOfField(2, "instructor_id", pJson["instructor_id"], err, false))
            return false;
    }
    return true;
}
...
Instructor Course::getInstructor(const drogon::orm::DbClientPtr &clientPtr) const {
...
void Course::getInstructor(const DbClientPtr &clientPtr,
...
std::vector<Review> Course::getReview(const drogon::orm::DbClientPtr &clientPtr) const {
    std::shared_ptr<std::promise<std::vector<Review>>> pro(new std::promise<std::vector<Review>>);
    std::future<std::vector<Review>> f = pro->get_future();
    getReview(clientPtr, [&pro] (std::vector<Review> result) {
        try {
            pro->set_value(result);
        }
        catch (...) {
...
void Course::getReview(const DbClientPtr &clientPtr,
                       const std::function<void(std::vector<Review>)> &rcb,
                       const ExceptionCallback &ecb) const
{
    const static std::string sql = "select * from review where course_id = $1";
    *clientPtr << sql
               << *id_
               >> [rcb = std::move(rcb)](const Result &r){
                   std::vector<Review> ret;
                   ret.reserve(r.size());
                   for (auto const &row : r)
                   {
                       ret.emplace_back(Review(row));
                   }
                   rcb(ret);
               }
               >> ecb;
