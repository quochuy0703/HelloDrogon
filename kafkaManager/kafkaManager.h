...
public:
    ...
    void initialize(const std::string &brokers)
    {
        ...
        if (!initialized_)
        {
            ...
            rd_kafka_conf_set(consumer_conf_, "max.poll.interval.ms", "70000", nullptr, 0);
    ...
    rd_kafka_t *createNewConsumer()
    {
        ...
        try
        {
            ...
            rd_kafka_conf_t *consumer_conf_temp = rd_kafka_conf_new();

            // enable.auto.commit: 自动提交偏移量的开关。生产环境中通常建议手动提交，以更好地控制偏移量的提交。
            rd_kafka_conf_set(consumer_conf_temp, "enable.auto.commit", "false", nullptr, 0);

            // auto.commit.interval.ms: 自动提交偏移量的时间间隔。如果 enable.auto.commit 设置为 true，则此配置项生效。
            rd_kafka_conf_set(consumer_conf_temp, "auto.commit.interval.ms", "5000", nullptr, 0);

            // 	session.timeout.ms: 消费者组会话超时。消费者在多长时间没有发送心跳时，协调器会将其从组中移除。生产环境中通常设置为 60s 或更长。
            rd_kafka_conf_set(consumer_conf_temp, "session.timeout.ms", "60000", nullptr, 0);
            rd_kafka_conf_set(consumer_conf_temp, "max.poll.interval.ms", "70000", nullptr, 0);

            // heartbeat.interval.ms: 心跳间隔。消费者发送心跳的频率。建议设置为 session.timeout.ms 的一半或更少，例如 3s。
            rd_kafka_conf_set(consumer_conf_temp, "heartbeat.interval.ms", "3000", nullptr, 0);

            // auto.offset.reset: 指定当没有初始偏移量时应该如何开始消费。earliest 表示从最早的消息开始。
            rd_kafka_conf_set(consumer_conf_temp, "auto.offset.reset", "earliest", nullptr, 0);

            // max.poll.records：控制每次拉取的记录数量。增加此值可以减少拉取请求的次数，但可能增加每次处理的负担：
            rd_kafka_conf_set(consumer_conf_temp, "max.poll.records", "1000", nullptr, 0);

            // fetch.min.bytes 和 fetch.max.wait.ms：这些配置影响消息的拉取频率和批量大小。可以尝试调整这些配置来优化性能：
            rd_kafka_conf_set(consumer_conf_temp, "fetch.min.bytes", "4096", nullptr, 0);
            rd_kafka_conf_set(consumer_conf_temp, "fetch.max.wait.ms", "200", nullptr, 0);

            std::string broker = "localhost:9092";
            if (rd_kafka_conf_set(consumer_conf_temp, "bootstrap.servers", broker.c_str(), errstr_, sizeof(errstr_)) != RD_KAFKA_CONF_OK)
            {
                throw std::runtime_error(std::string("Failed to configure Kafka consumer: ") + errstr_);
            }

            if (rd_kafka_conf_set(consumer_conf_temp, "group.id", "my_consumer_group", errstr_, sizeof(errstr_)) != RD_KAFKA_CONF_OK)
            {
                throw std::runtime_error(std::string("Failed to configure Kafka group ID: ") + errstr_);
            }
            ...
            consumer = rd_kafka_new(RD_KAFKA_CONSUMER, consumer_conf_temp, errstr_, sizeof(errstr_));
            if (!consumer)
            {
                throw std::runtime_error(std::string("Failed to create Kafka consumer: ") + errstr_);
            }
