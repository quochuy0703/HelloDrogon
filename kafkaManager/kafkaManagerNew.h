...
#ifndef kafkaManagerNew_H
#define kafkaManagerNew_H
...
class KafkaManagerNew
{
public:
    static KafkaManagerNew &instance()
    {
        static KafkaManagerNew instance;
        return instance;
    }
    ...
    void initialize(const std::string &brokers)
    {
        ...
        if (!initialized_)
        {
            ...
            props_ = new Properties({{"bootstrap.servers", {brokers}},
            ...
            consumer_ = new KafkaConsumer(*props_);
    ...
    rd_kafka_t *getProducer() const
    {
        if (!initialized_)
        {
            throw std::runtime_error("kafkaManagerNew is not initialized");
        }
        return producer_;
    }
    ...
    KafkaConsumer *getConsumer() const
    {
        if (!initialized_)
        {
            throw std::runtime_error("kafkaManagerNew is not initialized");
    ...
    KafkaConsumer *createNewConsumer()
    {
        ...
        try
        {
            ...
            // 创建新的消费者实例
            consumer = new KafkaConsumer(*props_);
            // if (!consumer)
            // {
            //     throw std::runtime_error(std::string("Failed to create Kafka consumer: ") + errstr_);
            // }
...
private:
    ...
    Properties *props_ = nullptr;
};

#endif // kafkaManagerNew_H
