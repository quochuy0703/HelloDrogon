...
private:
    void consumeMessages(rd_kafka_t *consumer_) const
    {
        ...
        // 向列表中添加主题和分区
        rd_kafka_topic_partition_list_add(partitions, "user-created-topic", RD_KAFKA_PARTITION_UA);
        // 订阅主题
    ...
    static Task<> handleKafkaMessage(const std::string &message)
    {
        try
        {
            ...
            Json::Value root;
            Json::Reader reader;
            auto json = reader.parse(std::string{message}, root);
            ...
            co_await app_services::usermicroservice_service::create(dto);
